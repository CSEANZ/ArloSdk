...
{
    public abstract class XWebRepo2<TEntity> : IWebRepo<TEntity> where TEntity : class, new()
    {
        ...
        public virtual T OnResultRetrieving<T>(string url, string data, string method)
        {
            return default(T);
        }

        public virtual T OnResultRetrieving<T>(string url, byte[] data, string method)
        {
            return default(T);
        }
        ...
        public virtual bool OnEntityRetreived(TEntity entity, string url, string data, string method)
        {
            return true;
        }

        public virtual bool OnEntityRetreived(TEntity entity, string url, byte[] data, string method)
        {
            return true;
        }
        ...
        {
            var url = Service + extra;

            var preCheck = OnResultRetrieving<TEntity>(url, data, method);

            if (preCheck != default(TEntity))
            {
                return preCheck;
            }

            var result = await _downloader.Upload(url, method, data);
            ...
            if (OnEntityRetreived(e, url, data, method))
            {
                return e;
            }
        ...
        private async Task<TEntity> _send(string serialisedData = null, string extra = null, string method = "POST")
        {
            ...
            var preCheck = OnResultRetrieving<TEntity>(url, serialisedData, method);
            if (preCheck != default(TEntity))
            {
                return preCheck;
            }
            ...
            if (OnEntityRetreived(e, url, serialisedData, method))
            {
                return e;
            }
